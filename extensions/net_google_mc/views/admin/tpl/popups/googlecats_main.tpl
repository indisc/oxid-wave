...
<script type="text/javascript">
    initAoc = function()
    {
        ...
        YAHOO.oxid.container1 = new YAHOO.oxid.aoc( 'container1',
                                                    [ [{ foreach from=$oxajax.container1 item=aItem key=iKey }]
                                                       ...
                                                       [{if !$aItem.4 }],
                                                       label: '[{ oxmultilang ident="NET_GENERAL_AJAX_SORT_"|cat:$aItem.0|oxupper }]',
                                                       visible: [{if $aItem.2 }]true[{else}]false[{/if}]
                                                       [{/if}]}
                                                      [{/strip}]
                                                      [{assign var="sSep" value=","}]
                                                      [{ /foreach }] ],
                                                    '[{ $oViewConf->getAjaxLink() }]cmpid=container1&container=googlecats_main&synchoxid=[{ $oxid }]'
                                                    );
        ...
        YAHOO.oxid.container2 = new YAHOO.oxid.aoc( 'container2',
                                                    [ [{ foreach from=$oxajax.container2 item=aItem key=iKey }]
                                                      ...
                                                      [{assign var="sSep" value=","}]
                                                      [{ /foreach }] ],
                                                    '[{ $oViewConf->getAjaxLink() }]cmpid=container2&container=googlecats_main&oxid=[{ $oxid }]'
                                                    );
    /* Deactivated
    ...
    <table width="100%">
        ...
        <tr class="edittext">
            <td align="center"><b>[{ oxmultilang ident="NET_GOOGLECATS_MAIN_AJAX_ALLCATEGORIES" }]</b></td>
            <td align="center"><b>[{ oxmultilang ident="NET_GOOGLECATS_MAIN_AJAX_SHOPCATEGORIES" }]</b></td>
            <td></td>
        </tr>
        [{*<tr>
