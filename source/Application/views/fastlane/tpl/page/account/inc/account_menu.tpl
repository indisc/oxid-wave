<nav id="account_menu" class="account-menu">
    <ul class="list-group">
        [{block name="account_menu"}]
            ...
            [{/if}]
        <li class="list-group-item" [{if $active_link == "reviewlist"}] active[{/if}]>
            <a class="list-group-link" href="[{oxgetseourl ident=$oViewConf->getSelfLink()|cat:"cl=VetBonusHistoryController"}]" title="[{oxmultilang ident="MY_REVIEWS"}]">Vet Bonus Bestell Historie</a>
        </li>
        [{/block}]
    </ul>
</nav>
