...
function editThis(sID)
{
    ...
    oSearch = parent.list.document.getElementById("search");
    oSearch.sort.value = '';
    oSearch.fnc.value = '';
    oSearch.lstrt.value = '';
    oSearch.oxid.value = sID;
    oSearch.cl.value = 'sccp_cparticle_list';
...
function DeleteThisArticle(sID)
{
    ...
    if( blCheck == true)
    {
        ...
        oDeleteThisArticle.fnc.value='DeleteThisArticle';
...
<form name="DeleteThisArticle" id="DeleteThisArticle" action="[{$oViewConf->getSelfLink()}]" method="post">
    ...
    <input type="hidden" name="cl" value="sccp_cporder_article">
...
<tr id="art.[{$smarty.foreach.orderArticles.iteration}]">
    [{block name="admin_order_article_listitem"}]
        ...
        [{/if}]
        <td valign="top" class="[{$listclass}]">[{if $listitem->oxorderarticles__oxstorno->value != 1 && !$listitem->isBundle()}]<label for="aOrderArticles_[{$listitem->getId()}]_oxamount"></label><input type="text" id="aOrderArticles_[{$listitem->getId()}]_oxamount" name="aOrderArticles[[{$listitem->getId()}]][oxamount]" value="[{$listitem->oxorderarticles__oxamount->value}]" class="listedit">[{else}][{$listitem->oxorderarticles__oxamount->value}][{/if}]</td>
        ...
        <td valign="top" class="[{$listclass}]">[{if !$listitem->isBundle()}]<a href="Javascript:DeleteThisArticle('[{$listitem->oxorderarticles__oxid->value}]');" class="delete" [{if $readonly}]onclick="return false;"[{/if}] [{include file="help.tpl" helpid=item_delete}]></a>[{/if}]</td>
        <td valign="top" class="[{$listclass}]">[{if !$listitem->isBundle()}]<a href="Javascript:StornoThisArticle('[{$listitem->oxorderarticles__oxid->value}]');" class="pause" [{if $readonly}]onclick="return false;"[{/if}] [{include file="help.tpl" helpid=item_storno}]></a>[{/if}]</td>
    [{/block}]
</tr>
[{if $blWhite == "2"}]
[{assign var="blWhite" value=""}]
