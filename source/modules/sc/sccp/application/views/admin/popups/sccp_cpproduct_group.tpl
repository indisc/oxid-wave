    ...
    initAoc = function()
    {
        YAHOO.oxid.container1 = new YAHOO.oxid.aoc( 'container1',
                                                    [ [{foreach from=$oxajax.container1 item=aItem key=iKey }]
                                                      ...
                                                      [{/foreach}] ],
                                                    '[{$oViewConf->getAjaxLink()}]cmpid=container1&container=sinkacom_creditplusmodule_productgroup&synchoxid=[{$oxid}]'
                                                    );
        ...
        YAHOO.oxid.container2 = new YAHOO.oxid.aoc( 'container2',
                                                    [ [{foreach from=$oxajax.container2 item=aItem key=iKey }]
                                                      ...
                                                      [{assign var="sSep" value=","}]
                                                      [{/foreach}] ],
                                                    '[{$oViewConf->getAjaxLink()}]cmpid=container2&container=sinkacom_creditplusmodule_productgroup&oxid=[{$oxid}]'
                                                    );
        ...
        var oArtCatSelect = $('artcat');
        $E.addListener( oArtCatSelect, "change", YAHOO.oxid.container1.filterCat, oArtCatSelect );
    };
    ...
    <table width="100%">
        ...
        <tr>
            <td class="oxid-aoc-category">
                <select title="[{oxmultilang ident='SCCP_CPPRODUCT_GROUP_FILTER_CATEGORIES'}]" name="artcat" id="artcat">
                [{foreach from=$oCatTreeList->aList item=pcat}]
